...
QVector<RecordPtr> RecordMaster::get(const QString &tag, int limit, int offset)
{
    ...
    for(;;) {
        ...
        if (res == SQLITE_ROW) {
            ...
            auto record = RecordPtr::create(*Storage::getId(stmt, 0));

            record->type(type)
                  ->content(loadContent(type, Storage::getInt(stmt, 2)))
                  ->date(Storage::getDate(stmt, 3));
...
void RecordMaster::add(const RecordPtr &record)
{
    ...
    sqlite3_bind_int64(stmt, 3, record->content()->rowid());
